...
public class TargetVisitChecker {
    ...
    public static boolean isTargetWithinRange(final double[] latitudes, final double[] longitudes,
        ...
        if (LatLngUtils.distance(latitudes[targetIndex], longitudes[targetIndex], currentLatitude,
                currentLongitude) < range) {
            return true;
        }
        return false;
    }
    ...
    public static boolean checkSnakeRule(final double[] latitudes, final double[] longitudes,
        ...
        if ((path[0] == -1) || (path[1] == -1)) {
            ...
            for (int i = 0; i <= index - 1; i++) {
                int start = path[i];
                int second = path[i + 1];
                if (LineCrossDetector.linesCross(latitudes[start], longitudes[start], latitudes[second],
                        longitudes[second], latitudes[path[index]], longitudes[path[index]], latitudes[tryVisit],
                        longitudes[tryVisit])) {
                    return false;
                }
            }
