...
public final class GameActivity extends AppCompatActivity {
    ...
    public void onLocationUpdate(final double latitude, final double longitude) {
        ...
        int index = TargetVisitChecker.getVisitCandidate(targetLats, targetLngs, path, latitude,
                longitude, PROXIMITY_THRESHOLD);
        if (index != -1) {
            if (TargetVisitChecker.checkSnakeRule(targetLats, targetLngs, path, index)) {
                int pathIndex = TargetVisitChecker.visitTarget(path, index);
                changeMarkerColor(targetLats[index], targetLngs[index], CAPTURED_MARKER_HUE);
                if (pathIndex != 0) {
                    addLine(targetLats[path[pathIndex - 1]], targetLngs[path[pathIndex - 1]], targetLats[index],
                            targetLngs[index], PLAYER_COLOR);
                }
            }
        }
